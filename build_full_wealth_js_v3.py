    ...
    container.innerHTML = `
      <div class="heatmap-cards-grid">
        ${items.map(it => {
          ...
          return `
            <div class="heatmap-card-item" style="background:${color};border:1px solid ${borderColor};"
              onclick="$('#searchInput').value='${it.name}';renderHoldings(dashboard);">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
                <strong style="font-size:13px;font-weight:700;color:#f3f5ff;">${html(it.name)}</strong>
                <span class="muted" style="font-size:11px;">${number(it.weight, 1)}%</span>
              </div>
